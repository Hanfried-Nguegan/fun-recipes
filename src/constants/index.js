import {
  cake,
  pancakes,
  pizza,
  blueberry,
  beef,
  spaghetti,
  shrimp,
  ndole,
} from "../assets";

export const navLinks = [
  {
    id: "all recipes",
    title: "ALL RECIPES",
  },
  {
    id: "story",
    title: "STORY",
  },
  {
    id: "about us",
    title: "ABOUT US",
  },
  {
    id: "contact",
    title: "CONTACT",
  },
];

export const recipesDataRight = [
  {
    id: 1,
    image: pancakes,
    title: "Pancakes",
    timer: "30",
    description: "Pancakes with butter and eggs",
    ingredients: [
      "1 cup fresh blueberries",
      "1 cup heavy cream",
      "1 cup whole milk",
      "3/4 cup granulated sugar",
      "1 teaspoon vanilla extract",
      "Pinch of salt",
    ],
    recipe: [
      "Preheat your oven to 350°F (180°C). Grease and flour two 9-inch round cake pans, or line them with parchment paper.",
      "In a medium bowl, whisk together the flour, baking powder, baking soda, and salt.",
      "In a large bowl, cream the butter and sugar together until light and fluffy, using an electric mixer on medium speed. Add the eggs one at a time, beating well after each addition. Stir in the vanilla extract.",
      "Add the dry ingredients to the butter mixture in three parts, alternating with the milk, beginning and ending with the dry ingredients. Mix until just combined.",
      "Divide the batter evenly between the prepared pans.",
      "Bake for 25 to 30 minutes, or until a toothpick inserted into the center of the cakes comes out clean.",
      "Let the cakes cool in their pans for 5 minutes, then remove them from the pans and transfer to wire racks to cool completely.",
      "Once the cakes are cool, you can frost and decorate them as desired. Enjoy your homemade vanilla cake!",
    ],
  },
  {
    id: 2,
    image: cake,
    title: "Cake",
    timer: "50",
    description: "Homemade Raspberry Cake",
    ingredients: [
      "1 cup fresh blueberries",
      "1 cup heavy cream",
      "1 cup whole milk",
      "3/4 cup granulated sugar",
      "1 teaspoon vanilla extract",
      "Pinch of salt",
    ],
    recipe: [
      "Preheat your oven to 350°F (180°C). Grease and flour two 9-inch round cake pans, or line them with parchment paper.",
      "In a medium bowl, whisk together the flour, baking powder, baking soda, and salt.",
      "In a large bowl, cream the butter and sugar together until light and fluffy, using an electric mixer on medium speed. Add the eggs one at a time, beating well after each addition. Stir in the vanilla extract.",
      "Add the dry ingredients to the butter mixture in three parts, alternating with the milk, beginning and ending with the dry ingredients. Mix until just combined.",
      "Divide the batter evenly between the prepared pans.",
      "Bake for 25 to 30 minutes, or until a toothpick inserted into the center of the cakes comes out clean.",
      "Let the cakes cool in their pans for 5 minutes, then remove them from the pans and transfer to wire racks to cool completely.",
      "Once the cakes are cool, you can frost and decorate them as desired. Enjoy your homemade vanilla cake!",
    ],
  },

  {
    id: 3,
    image: pizza,
    title: "Pizza",
    timer: "40",
    description: "Homemade Pizza with chicken and peperroni",
    ingredients: [
      "1 cup fresh blueberries",
      "1 cup heavy cream",
      "1 cup whole milk",
      "3/4 cup granulated sugar",
      "1 teaspoon vanilla extract",
      "Pinch of salt",
    ],
    recipe: [
      "Preheat your oven to 350°F (180°C). Grease and flour two 9-inch round cake pans, or line them with parchment paper.",
      "In a medium bowl, whisk together the flour, baking powder, baking soda, and salt.",
      "In a large bowl, cream the butter and sugar together until light and fluffy, using an electric mixer on medium speed. Add the eggs one at a time, beating well after each addition. Stir in the vanilla extract.",
      "Add the dry ingredients to the butter mixture in three parts, alternating with the milk, beginning and ending with the dry ingredients. Mix until just combined.",
      "Divide the batter evenly between the prepared pans.",
      "Bake for 25 to 30 minutes, or until a toothpick inserted into the center of the cakes comes out clean.",
      "Let the cakes cool in their pans for 5 minutes, then remove them from the pans and transfer to wire racks to cool completely.",
      "Once the cakes are cool, you can frost and decorate them as desired. Enjoy your homemade vanilla cake!",
    ],
  },
];

export const recipesDataLeft = [
  {
    id: 1,
    image: beef,
    title: "Beef",
    timer: "50",
    description: "Slowly cooked Beef Brisket with BBQ Sauce",
    ingredients: [
      "1 cup fresh blueberries",
      "1 cup heavy cream",
      "1 cup whole milk",
      "3/4 cup granulated sugar",
      "1 teaspoon vanilla extract",
      "Pinch of salt",
    ],
    recipe: [
      "Preheat your oven to 350°F (180°C). Grease and flour two 9-inch round cake pans, or line them with parchment paper.",
      "In a medium bowl, whisk together the flour, baking powder, baking soda, and salt.",
      "In a large bowl, cream the butter and sugar together until light and fluffy, using an electric mixer on medium speed. Add the eggs one at a time, beating well after each addition. Stir in the vanilla extract.",
      "Add the dry ingredients to the butter mixture in three parts, alternating with the milk, beginning and ending with the dry ingredients. Mix until just combined.",
      "Divide the batter evenly between the prepared pans.",
      "Bake for 25 to 30 minutes, or until a toothpick inserted into the center of the cakes comes out clean.",
      "Let the cakes cool in their pans for 5 minutes, then remove them from the pans and transfer to wire racks to cool completely.",
      "Once the cakes are cool, you can frost and decorate them as desired. Enjoy your homemade vanilla cake!",
    ],
  },
  {
    id: 2,
    image: spaghetti,
    title: "Spaghetti",
    timer: "30",
    description: "Spaghetti Bolognaise with tomato sauce",
    ingredients: [
      "1 cup fresh blueberries",
      "1 cup heavy cream",
      "1 cup whole milk",
      "3/4 cup granulated sugar",
      "1 teaspoon vanilla extract",
      "Pinch of salt",
    ],
    recipe: [
      "Preheat your oven to 350°F (180°C). Grease and flour two 9-inch round cake pans, or line them with parchment paper.",
      "In a medium bowl, whisk together the flour, baking powder, baking soda, and salt.",
      "In a large bowl, cream the butter and sugar together until light and fluffy, using an electric mixer on medium speed. Add the eggs one at a time, beating well after each addition. Stir in the vanilla extract.",
      "Add the dry ingredients to the butter mixture in three parts, alternating with the milk, beginning and ending with the dry ingredients. Mix until just combined.",
      "Divide the batter evenly between the prepared pans.",
      "Bake for 25 to 30 minutes, or until a toothpick inserted into the center of the cakes comes out clean.",
      "Let the cakes cool in their pans for 5 minutes, then remove them from the pans and transfer to wire racks to cool completely.",
      "Once the cakes are cool, you can frost and decorate them as desired. Enjoy your homemade vanilla cake!",
    ],
  },
  {
    id: 3,
    image: shrimp,
    title: "Shrimp",
    timer: "40",
    description: "Creamy garlic shrimp",
    ingredients: [
      "1 cup fresh blueberries",
      "1 cup heavy cream",
      "1 cup whole milk",
      "3/4 cup granulated sugar",
      "1 teaspoon vanilla extract",
      "Pinch of salt",
    ],
    recipe: [
      "Preheat your oven to 350°F (180°C). Grease and flour two 9-inch round cake pans, or line them with parchment paper.",
      "In a medium bowl, whisk together the flour, baking powder, baking soda, and salt.",
      "In a large bowl, cream the butter and sugar together until light and fluffy, using an electric mixer on medium speed. Add the eggs one at a time, beating well after each addition. Stir in the vanilla extract.",
      "Add the dry ingredients to the butter mixture in three parts, alternating with the milk, beginning and ending with the dry ingredients. Mix until just combined.",
      "Divide the batter evenly between the prepared pans.",
      "Bake for 25 to 30 minutes, or until a toothpick inserted into the center of the cakes comes out clean.",
      "Let the cakes cool in their pans for 5 minutes, then remove them from the pans and transfer to wire racks to cool completely.",
      "Once the cakes are cool, you can frost and decorate them as desired. Enjoy your homemade vanilla cake!",
    ],
  },
];

export const featuredDataRight = [
  {
    image: blueberry,
    title: "Blueberry Ice Cream",
    timer: "30",
    ingredients: [
      "1 cup fresh blueberries",
      "1 cup heavy cream",
      "1 cup whole milk",
      "3/4 cup granulated sugar",
      "1 teaspoon vanilla extract",
      "Pinch of salt",
    ],
  },
];

export const featuredDataLeft = [
  {
    image: ndole,
    title: "Ndole Plantain Mur",
    timer: "50",
    ingredients: [
      "500g of bitterleaf",
      "1kg of spinach",
      "2 tablespoons of crayfish",
      "1kg of meat",
      "2 onions",
      "1 cup of groundnuts",
      "1 cup of palmoil",
    ],
  },
];
